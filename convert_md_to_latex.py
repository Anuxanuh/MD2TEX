...
def make_header(header_path: Path, font_dir: Path, font_name: str, font_file: Path | None) -> None:
	...
	if font_file is None:
		...
		bold_file = font_file.with_name(font_file.name.replace("Regular", "Bold"))
		italic_file = font_file.with_name(font_file.name.replace("Regular", "Italic"))
		bold_italic_file = font_file.with_name(font_file.name.replace("Regular", "BoldItalic"))
		if bold_file.exists():
...
def fix_hyperref_targets(tex_path: Path) -> None:
	...
	content = re.sub(r"\\hyperref\[(?:[0-9]+-)([^]]+)\]", r"\\hyperref[\1]", content)
	content = re.sub(r"\\autoref\[(?:[0-9]+-)([^]]+)\]", r"\\autoref[\1]", content)
...
def _bordered_colspec(spec: str) -> str:
	...
	count = len(columns)
	width = r"\dimexpr(\textwidth - 2\tabcolsep*%d - %d\arrayrulewidth)/%d\relax" % (count, count + 1, count)
...
def _replace_table_begins_with_bordered(content: str) -> str:
	...
	for match in pattern.finditer(content):
		...
		parts.append(f"\\begin{{{match.group(1)}}}{{{_bordered_colspec(spec)}}}")
		cursor = brace_end + 1
...
def _style_table_frame_and_header(block: str) -> str:
	...
	head_part = re.sub(
		...
		lambda m: m.group(1) + r"\textbf{" + m.group(2).strip() + "}" + m.group(3),
		head_part,
		flags=re.DOTALL,
	)
...
def add_full_table_borders(tex_path: Path) -> None:
	...
	content = content.replace(r"\begin{longtable}", "\\begingroup\\footnotesize\n\\begin{longtable}")
	content = content.replace(r"\end{longtable}", "\\end{longtable}\n\\endgroup")
	content = re.sub(r"\\begin\{longtable\}.*?\\end\{longtable\}", lambda m: _normalize_table_text_block(m.group(0)), content, flags=re.DOTALL)
	content = re.sub(r"\\begin\{longtable\}.*?\\end\{longtable\}", lambda m: _add_row_hlines_to_longtable(m.group(0)), content, flags=re.DOTALL)
	content = re.sub(r"\\begin\{longtable\}.*?\\end\{longtable\}", lambda m: _style_table_frame_and_header(m.group(0)), content, flags=re.DOTALL)
	tex_path.write_text(content, encoding="utf-8")
...
def compile_pdf(tex_path: Path) -> None:
	...
	cmd = ["xelatex", "-interaction=nonstopmode", "-halt-on-error", "-synctex=1", tex_path.name]
	for _ in range(2):
		run_command(cmd, cwd=output_dir)


def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(description="将 Markdown 转换为 .tex（默认）；可选编译生成 PDF。")
	parser.add_argument("-i", "--input", default="服务器设计方案.md", help="输入 Markdown 文件路径")
	...
	parser.add_argument("--with-pdf", action="store_true", help="额外编译并生成 PDF（默认不生成）")
...
def main() -> None:
	...
	output_tex = Path(args.tex).expanduser() if args.tex else input_path.with_suffix(".tex")
	if not output_tex.is_absolute():
		output_tex = (output_dir / output_tex) if output_dir else (input_path.parent / output_tex)

	output_pdf = Path(args.pdf).expanduser() if args.pdf else input_path.with_suffix(".pdf")
	if not output_pdf.is_absolute():
		output_pdf = (output_dir / output_pdf) if output_dir else (input_path.parent / output_pdf)

	header_path = output_tex.with_name("pandoc_xelatex_header.tex")

	font_file = find_font_file(FONT_DIR, FONT_FILE_NAME)
	if font_file is None:
		print(f"WARNING: 未在 {FONT_DIR} 中找到 {FONT_FILE_NAME} 字体文件，将仅使用字体名注册。", file=sys.stderr)
	else:
